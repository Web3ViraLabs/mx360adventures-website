  ...
  return (
    <section className="border-y border-border bg-card py-8">
      ...
      <div className="group relative mt-5 overflow-hidden">
        ...
        <div className="flex w-max animate-[marquee_28s_linear_infinite] items-center gap-12 group-hover:[animation-play-state:paused]">
          {row.map((p, i) => (
            <span
              key={`${p.name}-${i}`}
              className="flex shrink-0 items-center gap-1.5 font-display text-xl font-bold tracking-tight text-foreground/45"
            >
